...
public sealed class ToolCategoryServiceTests
{
    private readonly FakeToolCategoryRepository _repository = new();
    private readonly FakeToolCategoryQueries _queries = new();
    private readonly FakeUnitOfWork _unitOfWork = new();
    private readonly FakeDateTimeProvider _clock = new(new DateTime(2026, 7, 19, 10, 0, 0, DateTimeKind.Utc));
    private readonly ToolCategoryService _sut;

    public ToolCategoryServiceTests()
    {
    ...
    public async Task Create_commits_once_and_forwards_cancellation_token()
    {
        ...
        var dto = await _sut.CreateAsync(
            new CreateToolCategoryRequest("Encoding", "encoding", null, null, 1),
            Guid.NewGuid(),
            cts.Token);

        Assert.Equal("encoding", dto.Slug);
    ...
    {
        _repository.Seed(ToolCategory.Create(
            Guid.NewGuid(),
            "Encoding",
            "encoding",
            null,
        ...
        var ex = await Assert.ThrowsAsync<ToolboxException>(() =>
            _sut.CreateAsync(new CreateToolCategoryRequest("Other", "encoding", null, null, 0)));

        Assert.Equal(ToolboxApplicationErrorCodes.CategorySlugDuplicate, ex.Code);
