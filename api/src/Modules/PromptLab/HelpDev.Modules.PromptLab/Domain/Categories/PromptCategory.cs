...
public sealed class PromptCategory : AggregateRoot<Guid>
{
    ...
    private PromptCategory(Guid id)
        : base(id)
    {
    }
    ...
    public static PromptCategory Create(
        Guid id,
        string name,
        string slug,
        string? description,
        ...
        DateTime utcNow)
    {
        if (id == Guid.Empty)
        {
            throw new DomainException("Category id must not be empty.", PromptLabErrorCodes.CategoryNameInvalid);
        }

        var category = new PromptCategory(id)
        {
            IsActive = true,
            CreatedAtUtc = utcNow,
            UpdatedAtUtc = utcNow,
            Slug = PromptSlug.Create(
                slug,
                SlugMaxLength,
                PromptLabErrorCodes.CategorySlugRequired,
                PromptLabErrorCodes.CategorySlugInvalid),
        };
    ...
    public bool ChangeDisplayOrder(int displayOrder, DateTime utcNow)
    {
        if (displayOrder < 0)
        {
            throw new DomainException("Display order must be >= 0.", PromptLabErrorCodes.CategoryNameInvalid);
        }
    ...
    private bool ApplyDetails(string name, string? description, string? icon, int displayOrder, bool force)
    {
        ...
        var normalizedDescription = NormalizeOptional(description, DescriptionMaxLength, PromptLabErrorCodes.CategoryNameInvalid);
    ...
    private static string NormalizeName(string name)
    {
        ...
        {
            throw new DomainException("Category name is required.", PromptLabErrorCodes.CategoryNameRequired);
        }

        var trimmed = name.Trim();
        if (trimmed.Length > NameMaxLength)
        {
            throw new DomainException("Category name is invalid.", PromptLabErrorCodes.CategoryNameInvalid);
        }
    ...
    private static string? NormalizeIcon(string? icon)
    {
        ...
        if (trimmed.Length > IconMaxLength
            ...
            || trimmed.Contains('}', StringComparison.Ordinal))
        {
            throw new DomainException("Category icon must be a safe icon key.", PromptLabErrorCodes.CategoryNameInvalid);
        }
