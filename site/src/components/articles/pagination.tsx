...
/** Centered numbered pagination — active purple gradient. */
export function ArticlesPagination({ page, totalPages, onPageChange }: ArticlesPaginationProps) {
  if (totalPages <= 1) return null;
  const items = pageItems(page, totalPages);

  return (
    <nav className="mt-8 flex items-center justify-center gap-2" aria-label="صفحه‌بندی مقالات">
      <button
        type="button"
        disabled={page <= 1}
        onClick={() => onPageChange(page - 1)}
        aria-label="صفحه قبل"
        className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-white/[0.1] bg-[#0F1626] text-[#94A3B8] transition hover:text-white disabled:opacity-40"
      >
      ...
      {items.map((item, index) =>
        item === "ellipsis" ? (
          ...
          <button
            ...
            className={[
              "inline-flex h-9 min-w-9 items-center justify-center rounded-full px-2 text-[13px] font-bold transition",
              item === page
