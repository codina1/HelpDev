...
export function ArticlesPagination({ page, totalPages, onPageChange }: ArticlesPaginationProps) {
  ...
  return (
    <nav className="mt-0 flex items-center justify-center gap-1.5" aria-label="صفحه‌بندی مقالات">
      <button
        type="button"
        disabled={page <= 1}
      ...
      {items.map((item, index) =>
        item === "ellipsis" ? (
          <span key={`e-${index}`} className="px-1 text-[12px] text-[#64748B]">
            …
          </span>
        ) : (
          <button
            ...
            className={[
              "inline-flex h-8 min-w-8 items-center justify-center rounded-lg px-2 text-[12.5px] font-bold transition",
              item === page
                ? "bg-gradient-to-l from-[#7C3AED] to-[#6D28D9] text-white shadow-[0_0_12px_rgba(124,58,237,0.35)]"
