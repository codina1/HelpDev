...
export type ToolsFiltersState = {
  query: string;
  categories: Exclude<ToolCategoryId, "all">[];
  price: ToolPriceId;
  sort: ToolSortId;
};

export const DEFAULT_TOOLS_FILTERS: ToolsFiltersState = {
  query: "",
  categories: [],
  price: "all",
  sort: "newest",
...
type Props = {
  value: ToolsFiltersState;
  onChange: (next: ToolsFiltersState) => void;
};
...
/** Glass filter sidebar — search · categories · price · sort. */
export function ToolFilter({ value, onChange }: Props) {
  return (
    <aside
    ...
    >
      ...
      <div className="mt-6 space-y-6">
        <FilterGroup title="دسته‌بندی‌ها">
          {TOOL_SIDEBAR_CATEGORIES.map((item) => (
            <CheckRow
              key={item.id}
              label={item.label}
              count={item.count}
              checked={value.categories.includes(item.id)}
              onToggle={() =>
                onChange({ ...value, categories: toggleInList(value.categories, item.id) })
              }
            />
          ))}
        </FilterGroup>
        ...
        <FilterGroup title="قیمت">
          {TOOL_PRICE_OPTIONS.map((item) => {
            const checked = value.price === item.id;
            return (
              <label
                key={item.id}
              ...
              >
                <input
                  ...
                  name="tools-price"
                  checked={checked}
                  onChange={() => onChange({ ...value, price: item.id })}
                  className="sr-only"
                />
                <span
                  className={[
        ...
        <FilterGroup title="ترتیب نمایش">
          <select
            value={value.sort}
            onChange={(event) =>
              onChange({ ...value, sort: event.target.value as ToolSortId })
            }
